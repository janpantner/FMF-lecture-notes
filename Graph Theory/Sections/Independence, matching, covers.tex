...
\begin{definition}
    Let $M$ be a matching. A path is an \emph{$M$-alternating path}\index{$M$-alternating path}
    if the edges along the path alternate between $M$ and $\overline{M} = E \setminus M$.
\end{definition}

\begin{definition}
    An $M$-alternating path is called an \emph{$M$-augmenting path}\index{$M$-augmenting path}
